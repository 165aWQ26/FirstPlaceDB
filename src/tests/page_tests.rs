...
fn write_multiple_values() {
    ...
    page.write(Some(20), 1).unwrap();
    page.write(None, 2).unwrap();
    page.write(Some(40), 3).unwrap();
...
fn write_until_full() {
    ...
    for i in 0..Page::PAGE_SIZE {
        ...
        page.write(Some(i as i64), i).unwrap();
    }
...
fn write_beyond_capacity_fails() {
    ...
    assert_eq!(page.write(Some(999), Page::PAGE_SIZE), Err(PageError::Full));
}
...
fn boundary_values() {
    let mut page = Page::default();
    page.write(Some(i64::MAX), 0).unwrap();
    page.write(Some(i64::MIN), 1).unwrap();
    page.write(Some(0), 2).unwrap();
