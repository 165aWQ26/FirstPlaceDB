...
impl Table {
    ...
    pub fn restore(
        ...
        let indices: Vec<Index> = (0..num_columns)
            .map(|i| {
                if i == key_index {
                    Index::new_unique()
                } else {
    ...
    pub fn read(&self, rid: i64) -> Result<Vec<Option<i64>>, DbError> {
        ...
        self.page_ranges.read(&addr).map_err(DbError::Storage)
    }
    ...
    fn read_record_internal(
        ...
        let mut result = self
            .page_ranges
            .read_data(&base_addr, self.num_data_columns)
            .map_err(DbError::Storage)?;
    ...
    fn read_col(
        ...
        self.page_ranges
            .read_single(col, addr, range)
            .map_err(DbError::Storage)
    }

    #[inline]
    fn get_unmerged_tail(
        &self,
        rid: i64,
    ) -> Result<(PhysicalAddress, i64, Option<i64>), DbError> {
        ...
        let tps = base_addr.tps.load(Ordering::Acquire);
        if tail_rid <= tps {
    ...
    pub fn merge(&self) -> Result<(), DbError> {
        ...
        for &base_rid in &dirty {
            ...
            // Check there is an unmerged tail to process
            ...
            // Skip records whose tail chain is already fully below TPS
            let tps = base_addr.tps.load(Ordering::Acquire);
            if indirection <= tps {
                self.dirty_base_rids.remove(&base_rid);
                continue;
            ...
            new_addr.tps.store(indirection, Ordering::Release);
